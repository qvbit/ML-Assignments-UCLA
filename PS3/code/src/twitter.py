    ...
    with open(infile, 'rU') as fid :
        ...
        for i, line in enumerate(fid):
            ...
            for j, word in enumerate(word_list):
                if (word in lst): feature_matrix[i][j] = 1
...
def cv_performance(clf, X, y, kf, metric="accuracy"):
    ...
    for train_index, test_index in kf:
        ...
        perf_arr.append(performance(y_test, y_pred, metric = metric))
    score = (np.mean(perf_arr))
...
def select_param_linear(X, y, kf, metric="accuracy"):
    ...
    for c in C_range:
        ...
        perf_c = cv_performance(clf, X, y, kf, metric)
        perf_log[c] = perf_c
        
        
    best_c, best_perf = -1, -1
    for c, p_c in perf_log.items():
        if p_c > best_perf:
            best_c, best_perf = c, p_c
    
    print(best_c)
    print("The performance with this value of C was: %f" % best_perf)
    
    return best_c, perf_log
...
def main() :
    ...
    ### ========== TODO : START ========== ###
    # DONE part 1: split data into training (training + cross-validation) and testing set
