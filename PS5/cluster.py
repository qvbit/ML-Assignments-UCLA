    ...
    def medoid(self) :
        ...
        for p in self.points:
            for k in self.points:
                if k != p:
                    best_point_and_dist.keys()[i] = (p,k)
                    best_point_and_dist.values()[i] = p.distance(k)
                    i += 1
                    
        medoid = min(best_point_and_dist, key=best_point_and_dist.get)
